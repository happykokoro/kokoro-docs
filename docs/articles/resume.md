...
## One Developer. One Distributed Platform. Half a Million Lines of Code.
...
**Server administration and maintenance.** The infrastructure competency behind Kokoro Tech did not begin with Docker Compose and GitHub Actions — it began with a professional server administrator role in the pre-AI era, where every operation was manual. Jenkins managed the CI/CD pipelines; Kubernetes handled container orchestration and cluster management; SSH sessions were the primary interface to production; and shell commands were documented in physical notebooks, referenced during maintenance windows because there was no AI to ask. That discipline — knowing the exact command, understanding what it does, and having it written down — is the foundation that the current automated infrastructure is built on. The career arc runs from manual ops (Jenkins, K8s, SSH, notebook-documented commands) through graduate studies in enterprise PHP and Laravel, to the current AI-augmented solo founder approach where agent-orchestra runs 22 parallel Claude agents and 115 MCP tools expose the full trading platform to AI. The contrast is not incidental — it is the point. Knowing what the manual work felt like makes the value of the automation legible.
