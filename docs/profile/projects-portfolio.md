...
- `dsp-filters` (89 tests) — H-infinity filter (4-state: position/velocity/acceleration/jerk with innovation gating), UKF (nalgebra Cholesky sigma points), particle filter (importance resampling), Morlet wavelet CWT (20 log-spaced scales), Hilbert transform (analytic signal via FFT), Mellin transform, Fourier, EWMA, rolling statistics
...
- `prediction` — IMM filter (3-regime Kalman: Trending/Ranging/Volatile with different velocity models), RBPF (Rao-Blackwellized particle filter: per-particle regime + Kalman branch), stochastic-vol dual Kalman (price + log-vol coupled), expert aggregator (Hedge algorithm with multiplicative weights), conformal prediction (rolling calibration, 95% coverage), Nelder-Mead simplex optimizer for autotune
