...
The AI coding assistant market is growing rapidly and becoming structurally important to how software is written. GitHub Copilot, launched in 2022, reported over 1.8 million paid subscribers by late 2023 and has integrated into the workflows of developers at most Fortune 500 technology companies. Cursor reached $100 million ARR in 2024, becoming one of the fastest-growing developer tools in history. Cline, Aider, and Windsurf have each carved out communities of power users willing to pay for more control than Copilot provides.

Claude Code — Anthropic's terminal-based AI coding agent — occupies a different position in this landscape. Where Copilot provides inline autocomplete and Cursor provides a Copilot-augmented IDE, Claude Code operates as an autonomous agent that can read entire codebases, write tests, execute commands, and complete multi-step engineering tasks. It targets the developer who wants to delegate work, not just get suggestions.

This creates a new category: AI development automation, distinct from AI code completion. The market for this category is nascent but growing fast. Demand signals include the rapid adoption of Claude Code among senior engineers, the proliferation of MCP servers (hundreds published to the Anthropic-maintained registry within months of the protocol's release), and the emergence of competing products (Devin, SWE-agent, OpenHands) that all validate the thesis that AI can do significant autonomous engineering work.

Estimates for the broader developer productivity tools market range from $6.8 billion (2023) to $12.6 billion (2028) at roughly 13% CAGR. AI coding specifically is compressing timelines — the $12.6 billion estimate was originally a 2030 projection that has been revised forward as adoption accelerated in 2024.

### 3.2 The MCP Ecosystem

Anthropic introduced the Model Context Protocol in October 2024 as a standard for connecting AI models to external data sources and tools. By early 2025, MCP had been adopted by major development environments including Claude Code, Cursor, Windsurf, and Zed. The protocol standardizes how tools are described, called, and how their results are returned to the model — enabling any developer to publish a server that any MCP-compatible client can use.

The MCP ecosystem is currently in its early adoption phase. Most published servers are narrow integrations: a single database, a single SaaS API, a single file system operation. Complex, domain-rich servers with tens of tools are rare. Kokoro's 115 tools across two servers represent an edge case — a domain where the operator wants AI-mediated access to a large, structured API surface rather than a simple one-off integration.
